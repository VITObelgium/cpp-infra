...
ColorMap::ColorMap(const std::vector<Color>& clist)
: ColorMap(colorListToDict(clist))
{
}

ColorMap::ColorMap(const std::vector<ColorInfo>& clist)
: ColorMap(colorListToDict(clist))
{
}

ColorMap::ColorMap(const std::array<Color, 256>& cmap)
: _cmap(cmap)
{
}
...
const Color& ColorMap::getColor(float value) const noexcept
{
    return _cmap[static_cast<uint8_t>(std::round(value * 255))];
}

const Color& ColorMap::getColor(uint8_t value) const noexcept
{
    return _cmap[value];
}

uint8_t ColorMap::processBand(float value, const std::vector<ColorDict::Entry>& dict) const noexcept
{
    assert(dict.size() >= 2);
