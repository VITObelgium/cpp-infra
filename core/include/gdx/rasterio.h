...
template <typename T>
void read_raster_data(CutOut cut, const gdal::RasterDataSet& dataSet, T* data, int dataCols)
{
    ...
    dataSet.read_rasterdata<T>(1, std::max(0, cut.srcColOffset), std::max(0, cut.srcRowOffset), cut.cols, cut.rows, data, cut.cols, cut.rows, 0, dataCols * sizeof(T));
}

void read_raster_data(CutOut cut, const gdal::RasterDataSet& dataSet, const std::type_info& info, void* data, int dataCols);
    ...
    const GDALColorTable* ct = nullptr)
{
    ...
    if (driver.type() == gdal::RasterType::GeoTiff) {
        driverOptions.emplace_back("COMPRESS=LZW");
        driverOptions.emplace_back("NUM_THREADS=ALL_CPUS");
    }
...
void write_raster_data(
    ...
    const RasterMetadata& meta,
    const fs::path& filename,
    const std::vector<std::string>& driverOptions,
    const std::unordered_map<std::string, std::string>& metadataValues,
    const GDALColorTable* ct = nullptr)
{
...
static BBox metadata_bounding_box(const RasterMetadata& meta)
{
    BBox result;
...
inline CutOut intersect_metadata(const RasterMetadata& srcMeta, const RasterMetadata& dstMeta)
{
...
RasterMetadata cast_raster(const RasterMetadata& meta, gsl::span<const TSource> srcData, gsl::span<TDest> dstData)
{
 ...
 */
template <typename T>
RasterMetadata data_from_dataset(const gdal::RasterDataSet& dataSet, const RasterMetadata& extent, gsl::span<T> dstData)
{
    using namespace detail;
    ...
    if (isByte && dstMeta.nodata.has_value() && !inf::fits_in_type<T>(dstMeta.nodata.value())) {
        ...
        read_raster_data(cutOut, dataSet, tempData.data(), extent.cols);
        dstMeta = cast_raster<float, T>(dstMeta, tempData, dstData);
    } else {
        read_raster_data(cutOut, dataSet, dstData.data(), dstMeta.cols);
    }
...
template <typename T>
RasterMetadata read_raster_data(gdal::RasterDataSet& dataSet, gsl::span<T> dstData)
{
    return data_from_dataset(dataSet, dataSet.geometadata(), dstData);
}

template <typename T>
RasterMetadata read_raster_data(gdal::RasterDataSet& dataSet, const RasterMetadata& extent, gsl::span<T> dstData)
{
    return data_from_dataset(dataSet, extent, dstData);
}

template <typename StorageType, class RasterType>
void write_raster(const RasterType& ras, const RasterMetadata& meta, const fs::path& filename, const std::vector<std::string>& driverOptions = {}, const std::unordered_map<std::string, std::string>& metadataValues = {})
{
    using namespace detail;
...
void write_raster(const RasterType& ras, const RasterMetadata& meta, const fs::path& filename, const std::type_info& storageType, const std::vector<std::string>& driverOptions = {}, const std::unordered_map<std::string, std::string>& metadataValues = {})
{
    using namespace detail;
    inf::file::create_directory_if_not_exists(filename.parent_path());
...
template <class RasterType>
void write_raster(const RasterType& ras, const RasterMetadata& meta, const fs::path& filename, const std::vector<std::string>& driverOptions = {})
{
    write_raster<typename RasterType::value_type>(ras, meta, filename, driverOptions);
}

template <class RasterType>
void write_raster_with_metadata(const RasterType& ras, const RasterMetadata& meta, const fs::path& filename, const std::unordered_map<std::string, std::string>& metadataValues, const std::vector<std::string>& driverOptions = {})
{
    write_raster<typename RasterType::value_type>(ras, meta, filename, driverOptions, metadataValues);
}

template <class RasterType>
void write_raster_color_mapped(const RasterType& ras, const RasterMetadata& meta, const fs::path& filename, const inf::ColorMap& cm, const std::vector<std::string>& driverOptions = {})
{
    using namespace detail;
...
template <typename T>
inf::gdal::VectorDataSet polygonize(gsl::span<const T> rasterData, const RasterMetadata& meta)
{
...
void warp_raster(gsl::span<const TInput> inputData, const RasterMetadata& inputMeta, gsl::span<TOutput> outputData, RasterMetadata outputMeta, gdal::ResampleAlgorithm algo = gdal::ResampleAlgorithm::NearestNeighbour)
{
