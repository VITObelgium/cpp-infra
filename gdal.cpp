...
#ifdef WIN32
    // on windows the /vsimem/file.asc in memory path is translated to \\vsimem\\file.asc by fs::path
    // which is not recognized by gdal, translate it back to forward slashes
    if (path.find("\\vsimem\\") == 0) {
        for (auto& c : path) {
            if (c == '\\') {
                c = '/';
            }
