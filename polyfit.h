...
public:
    ...
    : _opts( opt )
  {
    ...
    qr.compute( _V );
    _R    = qr.matrixQR().topRows(_opts.order+1).template triangularView<Upper>();
    _Rinv = _R.inverse();
  ...
  */
  void eval( const std::vector<T>& x, std::vector<T>& yhat )
  {
    matrix_t V;   
  ...
  /*!
    ...
    /param yhat reference to the vector with y values to fill with the result
  */
  void eval( const std::vector<T>& x, std::vector<T>& yhat, std::vector<T>& yhaterr )
  {
    matrix_t V;   
  ...
  size_t npar( void ) { return _p.size(); }

  //! returns the parameter requested, idx corresponds to the power of x, i.e. idx = 0 is the constant
  T par( size_t idx ) { assert( idx >= 0); assert( idx < _p.size() ); return _p[idx]; }
