...
function(vcpkg_configure_cmake)
    cmake_parse_arguments(_csc "PREFER_NINJA;DISABLE_PARALLEL_CONFIGURE" "SOURCE_PATH;GENERATOR;INIT_CACHE" "OPTIONS;OPTIONS_DEBUG;OPTIONS_RELEASE" ${ARGN})
    ...
    if(_csc_INIT_CACHE)
        list(APPEND _local_OPTIONS -C ${_csc_INIT_CACHE})
    endif()

    list(APPEND _local_OPTIONS -G ${GENERATOR})
    ...
    if(DEFINED VCPKG_CMAKE_SYSTEM_NAME AND NOT VCPKG_CMAKE_SYSTEM_NAME STREQUAL CMAKE_HOST_SYSTEM_NAME)
        # only append system name if it does not match the host system name, otherwise we are cross compiling
        list(APPEND _local_OPTIONS "-DCMAKE_SYSTEM_NAME=${VCPKG_CMAKE_SYSTEM_NAME}")
        if(VCPKG_CMAKE_SYSTEM_NAME STREQUAL "WindowsStore" AND NOT DEFINED VCPKG_CMAKE_SYSTEM_VERSION)
            set(VCPKG_CMAKE_SYSTEM_VERSION 10.0)
        endif()
    endif()
    if(DEFINED VCPKG_CMAKE_SYSTEM_VERSION)
        list(APPEND _local_OPTIONS "-DCMAKE_SYSTEM_VERSION=${VCPKG_CMAKE_SYSTEM_VERSION}")
    endif()

    if(VCPKG_LIBRARY_LINKAGE STREQUAL "dynamic")
        list(APPEND _local_OPTIONS -DBUILD_SHARED_LIBS=ON)
    elseif(VCPKG_LIBRARY_LINKAGE STREQUAL "static")
        list(APPEND _local_OPTIONS -DBUILD_SHARED_LIBS=OFF)
    else()
    ...
    if(VCPKG_CHAINLOAD_TOOLCHAIN_FILE)
        list(APPEND _local_OPTIONS "-DVCPKG_CHAINLOAD_TOOLCHAIN_FILE=${VCPKG_CHAINLOAD_TOOLCHAIN_FILE}")
    endif()

    list(APPEND _local_OPTIONS
    ...
    if (VCPKG_CMAKE_SYSTEM_NAME STREQUAL "Emscripten")
        ...
        list(APPEND _local_OPTIONS "-DCMAKE_CROSSCOMPILING_EMULATOR=${NODEJS}")
    endif ()
    ...
    set(dbg_command
        ${CMAKE_COMMAND} "${_local_OPTIONS}" "${_csc_OPTIONS}" "${_csc_OPTIONS_DEBUG}"
