...
protected:
    ...
    {
        FileTools::del("clc06d_grid_4x4.txt");
    ...
    void configure()
    {
        auto configXml =
            "<?xml version=\"1.0\"?>"
            "  <config>"
            "    <file_pattern>./clc06d_grid_%grid%.txt</file_pattern>"
            "</config>"s;
...
TEST_F(TextGridProviderTest, GetGridCells)
{
    ...
    FileTools::writeTextFile("clc06d_grid_4x4.txt", ss.str());

    auto grid = _gridProvider.getGrid(GridType::Grid4x4);
...
{
    EXPECT_EQ(0, _gridProvider.getGrid(GridType::Grid4x4).cellCount());
}
