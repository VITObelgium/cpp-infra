...
namespace nnet {
  ...
  public:
    ...
    int size( void ) { return _a.size(); }
    std::string name( void ) { return _Name; }
    double* values( void ){ return _a.data(); } // the .data() method contains a const pointer to the scalar data
    Eigen::VectorXd& valuesAsVector( void ){ return _a; } 

    int inputSize( void ) { return _LW.cols(); }
    int outputSize( void ) { return _LW.rows(); }
