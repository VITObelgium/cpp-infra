      ...
      if ( ! kv.first.compare( "map" ) ) 
      {
        if ( ! kv.second.get<std::string>("<xmlattr>.grid").compare( cnf.grid() ) ) 
        {
          ...
          if (! _apscf.fmt.compare( "F7.1") )
            _apscf.c_fmt = "%7.1f";
          else throw std::runtime_error( "TODO: currently only F7.1 supported, need to implement flexible format" );
...
void apswriter::write_buffer( FILE* fp )
{
  ...
  {    
    for ( unsigned int j = 0; j< _apscf.nx; j++ ) fprintf( fp, _apscf.c_fmt.c_str(), _buffer[i*_apscf.nx+j] );
    fprintf(fp,"\n");
  }
...
void apswriter::write_header( FILE* fp, const boost::posix_time::ptime& curr_time, const std::string& comments )
{
  ...
  // write date & time
  fprintf( fp, "%3d%3d%3d%3ld ", ( d.tm_year > 100 ? d.tm_year-100 : d.tm_year ), d.tm_mon+1, d.tm_mday, curr_time.time_of_day().hours() );
