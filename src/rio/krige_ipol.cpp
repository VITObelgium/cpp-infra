...
krige_ipol::krige_ipol(const config& cf, const std::shared_ptr<rio::network>& net)
: mapper(cf, net)
, _logtrans(false)
{
    _name = "KRIGING";
    ...
    if (!spEl) {
        throw std::runtime_error("no correlation model specified in krige_ipol mapper");
    }
    ...
    if (!sp_type.compare("exp_spcorr")) {
        ...
        throw RuntimeError("requested spatial correlation model \"{}\" in krige_ipol is not supported...", sp_type);
    }
}
