  ...
  , _curr_sel("")
{
  ...
  std::cout << " Importing trend model config from : " << config_file << std::endl;
  TiXmlDocument xml;
  if ( ! xml.LoadFile( config_file ) ) throw std::runtime_error( "unable to load/parse xml file " + config_file );
  TiXmlElement *rootEl = xml.FirstChildElement( "trendmodel" ); 
  if ( ! rootEl ) throw std::runtime_error( "no <trendmodel> root element found in " + config_file );
  ...
  if ( model_class.compare( "polytrend" ) )
       throw std::runtime_error( "trend model config type does not match, expected polytrend" );

  // Look up the parameters into boost property tree
  TiXmlElement *trendEl = rootEl->FirstChildElement( "trend" );
  if ( ! trendEl ) throw std::runtime_error( "no <trend> element found in " + config_file );
  std::stringstream ss;
  ss << (*trendEl);
  pt::read_xml( ss, _trend, pt::xml_parser::trim_whitespace | pt::xml_parser::no_comments );

  // Look up the station statistics
  TiXmlElement *stEl = rootEl->FirstChildElement( "stat_param" );
  if ( ! stEl ) throw std::runtime_error( "no <stat_param> element found in " + config_file );
  
  for ( TiXmlElement* e = stEl->FirstChildElement("station");
	e ; e = e->NextSiblingElement("station") ) {
    
    std::string stName = e->Attribute("name");
    ss << (*e);
  ...
  */
  ...
  TiXmlElement *cnfEl = rootEl->FirstChildElement( "config" );
  if ( ! cnfEl ) throw std::runtime_error( "no <config> element found in " + config_file );
  ss << (*cnfEl);
  pt::ptree t;
  try {
    ...
    std::runtime_error( "unable to read config from trend xml file" );
  }
...
void polytrend::select( const std::string& aggr, boost::posix_time::ptime tstart )
{
  ...
  if ( selection.compare( _curr_sel ) )
  {
    ...
    try {
      ...
      throw std::runtime_error( std::string( "cannot retrieve trend parameters : " ) + e.what() );
    }
...
{
  double y = p[0];
  for ( int i = 1; i < p.size(); i++ ) y = y*x + p[i];
  return y;
}
...
double polytrend::poly( const std::vector<double>& p, double x, double x_lo, double x_hi )
{
  ...
  if ( x_hi <= x_lo ) throw std::runtime_error( "x_lo,x_hi boundaries not correct" );
  
  switch ( p.size() ) {
  case 0:
  case 1:
    throw std::runtime_error( "Invalid polynomial degree (<1)" );
    break;
