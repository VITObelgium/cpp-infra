...
switch lower(mode)
    case 'apply'        
        ...
        % BM: not really tested yet...        
        y = bsxfun( @plus, bsxfun( @rdivide, ...
            bsxfun( @times, PS.ymax-PS.ymin, bsxfun( @minus, x, PS.xmin ) ), ...
            PS.xmax-PS.xmin), PS.ymin );
    case 'reverse'        
        % here y is in fact x and vice versa
        %y = (PS.xmax-PS.xmin).*(x-PS.ymin)./(PS.ymax-PS.ymin) + PS.xmin;
        y = bsxfun( @plus, bsxfun( @rdivide, ...
            bsxfun( @rdivide, PS.xmax-PS.xmin, bsxfun( @minus, x, PS.ymin ) ), ...
            PS.ymax-PS.ymin), PS.xmin );
    otherwise
