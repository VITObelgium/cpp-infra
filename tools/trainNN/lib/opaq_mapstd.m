...
switch lower(mode)
    case 'apply'        
        ...
        y = bsxfun(@plus, bsxfun( @times, bsxfun( @minus, x, PS.xmean), PS.ystd./PS.xstd ), PS.ymean );
    case 'reverse'
        %y = ( x-PS.ymean)./(PS.ystd./PS.xstd) + PS.xmean;
        y = bsxfun(@plus, bsxfun( @rdivide, bsxfun( @minus, x, PS.ymean), PS.ystd./PS.xstd), PS.xmean );
    otherwise
